...
readFile(packageJsonFile, { encoding: 'utf8' }, (error, data) => {
  ...
  // Il faudra récupérer le port dans le fichier package.json pour le serveur local.
  const options = {
    'standalone': true,
    'doctype': 'article',
    'safe': 'unsafe',
  ...
  copyToBuild.forEach(folder => {
    let destination = \`./build/\${folder.slice(10)}/\`;

    // Get files in a source folder
    readdir(folder, (error, files) => {
      files.forEach((error, file) => {
        // If not hidden file
        if (files[file].charAt(0) !== '.') {
          // Create the folder structure
          mkdir(destination, { recursive: true }, () => {
            let filecopied = \`\${folder}/\${files[file]}\`;

            // Get the modification time of the destination file
            stat(\`\${destination}/\${files[file]}\`, (error, stats) => {
              let destFile = (!error) ? stats.mtimeMs : 0;

              // Get the modification time of the source file
              stat(filecopied, (error, stats) => {
                let srcFile = stats.mtimeMs;

                // If the source it's newer from the destination, then copy file to the destination folder
                if (destFile < srcFile) {
                  copyFile(filecopied, \`\${destination}/\${files[file]}\`);
